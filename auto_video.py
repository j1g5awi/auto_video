...
def reset_gui():
    ...
    for i in setting["character"]:
        tv.insert(
            ...
            values=(
                i,
                setting["character"][i]["sound"]["spd"],
                setting["character"][i]["sound"]["per"],
                setting["character"][i]["sound"]["pid"],
                setting["character"][i]["location"],
            ),
        )
        ans += 1
    tv.update()
...
def select_file():
    ...
    log_file = filedialog.askopenfilenames(
        title="请选择剧本文件", filetypes=[("Text", "*.txt"), ("All Files", "*")]
    )[0]
